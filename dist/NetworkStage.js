...
function fibonacciSpherePoints(count, radius, rng) {
...
`;
export default function NetworkStage(props) {
    const { rootRef, size, input, seed, reducedMotion, labels, nodeColor, accentColor, nodeSize, nodeMetalness, nodeRoughness, showLabels, labelStyle, labelTextColor, labelBgColor, labelBorderColor, labelGlowColor, labelScale, labelOpacity, labelMinOpacity, labelOffsetX, labelOffsetY, connectionDensity, lineColor, lineOpacity, lineWidth, pulseColor, pulseSpeed, pulseIntensity, pulsesPerLine, autoRotate, driftSpeed, pointerParallax, cameraDistance, transparentBackground, } = props;
    const canvasRef = React.useRef(null);
    const labelLayerRef = React.useRef(null);
    const rendererRef = React.useRef(null);
    const sceneRef = React.useRef(null);
    const cameraRef = React.useRef(null);
    const cameraRigRef = React.useRef(null);
    const nodeInstRef = React.useRef(null);
    const nodeGeoRef = React.useRef(null);
    const nodeMatRef = React.useRef(null);
    const nodePositionsRef = React.useRef(null);
    const nodeColorTintRef = React.useRef(null); // 0/1 per node: use accent?
    const linesRef = React.useRef(null);
    const lineGeoRef = React.useRef(null);
    const lineMatRef = React.useRef(null);
    const baseLinesRef = React.useRef(null);
    const baseLineGeoRef = React.useRef(null);
    const baseLineMatRef = React.useRef(null);
    ...
    const labelRefsRef = React.useRef([]);
    ...
    React.useEffect(() => {
        ...
        lineGeo.attributes.aU.needsUpdate = true;
    ...
    React.useEffect(() => {
        ...
        if (baseLineMat)
            baseLineMat.resolution.set(w, h);
    ...
    React.useEffect(() => {
        ...
        if (!m)
            return;
        m.uniforms.u_lineColor.value.set(lineColor);
        if (base)
            base.color.set(lineColor);
    }, [lineColor]);
    React.useEffect(() => {
        const m = lineMatRef.current;
        if (m)
            m.uniforms.u_lineOpacity.value = lineOpacity;
        const base = baseLineMatRef.current;
        if (base)
            base.opacity = Math.max(0, Math.min(1, lineOpacity * 0.8));
    }, [lineOpacity]);
    React.useEffect(() => {
        const base = baseLineMatRef.current;
        if (!base)
    ...
    React.useEffect(() => {
        ...
        if (!m)
            return;
        m.uniforms.u_pulseColor.value.set(pulseColor);
    }, [pulseColor]);
    React.useEffect(() => {
        const m = lineMatRef.current;
        if (!m)
    ...
    React.useEffect(() => {
        ...
        const phaseAttr = lineGeo.attributes.aPhase;
        const seedAttr = lineGeo.attributes.aSeed;
        const posArr = posAttr.array;
        const phaseArr = phaseAttr.array;
        const seedArr = seedAttr.array;
    ...
    useHeroAnimationFrame(rootRef, ({ delta }) => {
        ...
        const positions = nodePositionsRef.current;
        if (!renderer || !scene || !camera || !rig || !lineMat)
            return;
    ...
    return (_jsxs(_Fragment, { children: [_jsx("canvas", { ref: canvasRef, className: "crazygl-anm-canvas", "aria-hidden": "true" }), showLabels ? (_jsx("div", { ref: labelLayerRef, className: "crazygl-anm-labels", "data-label-style": labelStyle || 'glass', style: {
                    ['--anm-label-text']: labelTextColor,
                    ['--anm-label-bg']: labelBgColor,
                    ['--anm-label-border']: labelBorderColor,
                    ['--anm-label-glow']: labelGlowColor,
                }, "aria-hidden": "true", children: labels.slice(0, nodeCount).map((label, i) => (_jsx("span", { ref: (el) => {
                        labelRefsRef.current[i] = el;
                    }, className: "crazygl-anm-label", style: { transform: 'translate3d(-1000px, -1000px, 0)' }, children: label }, `${label}-${i}`))) })) : null] }));
}
