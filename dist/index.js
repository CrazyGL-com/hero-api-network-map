...
function ApiNetworkMapHero(props) {
    const { size, input, seed, reducedMotion, rootRef, 
    // CTA
    ctaLabel = 'View the map →', ctaTextColor = '#03060e', ctaBgColor = '#dfeaff', ctaGlowColor = '#5d8bff', onCTAClick = '', 
    // Nodes
    nodeLabels = 'auth\npayments\ndb\nqueue\nedge\ngateway\ncache\nstorage\nsearch\nworkers\nwebhooks\nanalytics', nodeColor = '#bfd6ff', accentColor = '#7aa9ff', nodeSize = 0.18, nodeMetalness = 0.55, nodeRoughness = 0.28, showLabels = true, labelStyle = 'glass', labelTextColor = '#f4f8ff', labelBgColor = '#071020', labelBorderColor = '#9cc2ff', labelGlowColor = '#5d8bff', labelScale = 1.12, labelOpacity = 1, labelMinOpacity = 0.62, labelOffsetX = 12, labelOffsetY = 10, 
    // Connections
    connectionDensity = 0.45, lineColor = '#5d8bff', lineOpacity = 0.45, lineWidth = 2.2, pulseColor = '#aee0ff', pulseSpeed = 0.45, pulseIntensity = 1.2, pulsesPerLine = 2, 
    // Camera
    autoRotate = true, driftSpeed = 0.06, pointerParallax = 0.35, cameraDistance = 6.0, 
    // Atmosphere
    bgTop = '#0a1228', bgBottom = '#03060e', transparentBackground = false, } = props;
    ...
    const showCta = ctaLabel && props.contentType !== 'custom';
    ...
    return (_jsxs(_Fragment, { children: [_jsx("crazygl-stage", { style: {
                ...
                }, children: _jsx(NetworkStage, { rootRef: rootRef, size: size, input: input, seed: seed, reducedMotion: reducedMotion, labels: labels, nodeColor: nodeColor, accentColor: accentColor, nodeSize: nodeSize, nodeMetalness: nodeMetalness, nodeRoughness: nodeRoughness, showLabels: showLabels, labelStyle: labelStyle, labelTextColor: labelTextColor, labelBgColor: labelBgColor, labelBorderColor: labelBorderColor, labelGlowColor: labelGlowColor, labelScale: labelScale, labelOpacity: labelOpacity, labelMinOpacity: labelMinOpacity, labelOffsetX: labelOffsetX, labelOffsetY: labelOffsetY, connectionDensity: connectionDensity, lineColor: lineColor, lineOpacity: lineOpacity, lineWidth: lineWidth, pulseColor: pulseColor, pulseSpeed: pulseSpeed, pulseIntensity: pulseIntensity, pulsesPerLine: pulsesPerLine, autoRotate: autoRotate, driftSpeed: driftSpeed, pointerParallax: pointerParallax, cameraDistance: cameraDistance, transparentBackground: transparentBackground }) }), _jsx("crazygl-content", { style: {
                    position: 'absolute',
                    inset: 0,
                    zIndex: 1,
                    display: 'flex',
                    alignItems: 'center',
                    pointerEvents: 'none',
                }, children: _jsxs("div", { className: "crazygl-anm-content", children: [content.node, showCta ? (_jsx("div", { className: "crazygl-anm-cta-row", children: _jsx("button", { type: "button", onClick: handleCTAClick, className: "crazygl-anm-cta", style: {
                                    color: ctaTextColor,
...
export { metadata };
export default function ApiNetworkMap(props) {
    return _jsx(CrazyGLWrapper, { hero: ApiNetworkMapHero, metadata: metadata, ...props });
}
